...
export function HeroSection() {
  ...
  const Play = () => (
    <svg
      className={`h-5 w-5 group-hover:scale-110 transition-transform ${isRTL ? "ml-2" : "mr-2"}`}
      fill="none"
      stroke="currentColor"
      viewBox="0 0 24 24"
    >
  ...
  return (
    <section className="min-h-screen flex items-center justify-center px-4 py-20 relative" dir={isRTL ? "rtl" : "ltr"}>
      <div className="max-w-4xl mx-auto text-center relative z-10 animate-fade-in-hero">
        {/* Badge */}
        <div
          className={`inline-flex items-center px-4 py-2 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-white text-sm font-medium mb-8 mt-12 animate-fade-in-badge ${isRTL ? "flex-row-reverse" : ""}`}
        >
          <span className={`w-2 h-2 bg-white/60 rounded-full animate-pulse ${isRTL ? "ml-2" : "mr-2"}`}></span>
          {t.hero.badge}
        </div>
        ...
        <h1 className="text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-bold text-balance mb-6 animate-fade-in-heading">
          ...
          <br />
          <span
            className={`inline-flex items-center justify-center flex-wrap gap-2 mt-4 sm:mt-6 md:mt-8 ${isRTL ? "flex-row-reverse" : ""}`}
          >
        ...
        {/* CTA Buttons */}
        <div
          className={`flex flex-col sm:flex-row items-center justify-center gap-4 mb-8 sm:mb-16 animate-fade-in-buttons ${isRTL ? "sm:flex-row-reverse" : ""}`}
        >
          <Button
            size="lg"
            className={`bg-white text-black rounded-full px-8 py-4 text-lg font-medium transition-all duration-300 hover:bg-gray-50 hover:scale-105 hover:shadow-lg group cursor-pointer relative overflow-hidden ${isRTL ? "flex-row-reverse" : ""}`}
          >
            {t.hero.cta1}
            <ArrowIcon />
          </Button>
          ...
          <Button
            variant="outline"
            size="lg"
            className={`rounded-full px-8 py-4 text-lg font-medium border-border hover:bg-accent transition-all duration-200 hover:scale-105 group bg-transparent cursor-pointer ${isRTL ? "flex-row-reverse" : ""}`}
          >
            <Play />
            {t.hero.cta2}
          </Button>
        ...
        <div className="text-center px-4 hidden sm:block overflow-hidden animate-fade-in-trust">
          <p className="text-sm text-white mb-6">{t.hero.trustText}</p>
